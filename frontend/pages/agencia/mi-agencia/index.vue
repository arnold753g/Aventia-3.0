<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <template v-else>
      <Card v-if="activeTab === 'general'" class="mb-6">
        <template #title>
          <div class="flex items-center gap-2">
            <i class="pi pi-share-alt text-blue-600"></i>
            <span>Compartir mi agencia</span>
          </div>
        </template>
        <template #content>
          <div class="space-y-3">
            <p class="text-sm text-gray-600">
              Comparte el enlace público de tu agencia en redes sociales para que más personas puedan conocer tus servicios y paquetes.
            </p>
            <AgenciasShareAgencyButton :slug="agencia?.slug" />

            <div v-if="estadisticasVisitas" class="mt-4 pt-4 border-t border-gray-200">
              <p class="text-sm font-semibold text-gray-700 mb-2">Estadísticas de visitas</p>
              <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                <div class="p-3 bg-blue-50 rounded-lg">
                  <p class="text-2xl font-bold text-blue-600">{{ estadisticasVisitas.total_visitas || 0 }}</p>
                  <p class="text-xs text-gray-600">Total visitas</p>
                </div>
                <div class="p-3 bg-green-50 rounded-lg">
                  <p class="text-2xl font-bold text-green-600">{{ estadisticasVisitas.visitas_ultima_semana || 0 }}</p>
                  <p class="text-xs text-gray-600">Última semana</p>
                </div>
                <div class="p-3 bg-purple-50 rounded-lg">
                  <p class="text-2xl font-bold text-purple-600">{{ estadisticasVisitas.visitas_ultimo_mes || 0 }}</p>
                  <p class="text-xs text-gray-600">Último mes</p>
                </div>
                <div class="p-3 bg-orange-50 rounded-lg">
                  <p class="text-2xl font-bold text-orange-600">{{ estadisticasVisitas.dias_con_visitas || 0 }}</p>
      ...
      <div v-if="activeTab === 'general'" class="mt-6">
        ...
        <AgenciasAgenciaPublicView v-if="agencia" :agencia="agencia" :agenciaId="agencia?.id" />
...
const { getEstadisticas } = useAgenciaVisitas()

const agencia = ref<any>(null)
const estadisticasVisitas = ref<any>(null)
...
const loadAgencia = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      await Promise.all([loadPoliticas(Number(response.data.id)), loadDatosPago(Number(response.data.id)), loadCapacidad(Number(response.data.id)), loadEstadisticas(Number(response.data.id))])
    } else {
      agencia.value = null
    }
  } catch (error: any) {
...
const loadEstadisticas = async (agenciaId: number) => {
  try {
    const data = await getEstadisticas(agenciaId)
    estadisticasVisitas.value = data
  } catch (error) {
    // Silenciosamente fallar - las estadísticas son opcionales
    estadisticasVisitas.value = null
  }
}
