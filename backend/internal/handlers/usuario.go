...
const minSubscriberDigits = 4
const maxSubscriberDigits = 14
...
func matchPhonePrefix(phone string) string {
	for _, prefix := range allowedPhonePrefixes {
...
func validatePhone(input string) (string, bool) {
	...
	if strings.HasPrefix(normalized, "+") {
		if len(normalized) == 1 || !isOnlyDigits(normalized[1:]) {
			return "", false
		}
		prefix := matchPhonePrefix(normalized)
		if prefix == "" {
			return "", false
		}
		number := normalized[len(prefix):]
		if number == "" || !isOnlyDigits(number) {
			return "", false
		}
		if len(number) < minSubscriberDigits || len(number) > maxSubscriberDigits {
			return "", false
		}
		return normalized, true
	}

	if !isOnlyDigits(normalized) {
		return "", false
	}
	if len(normalized) < minSubscriberDigits || len(normalized) > maxSubscriberDigits {
		return "", false
	}
...
// CreateAgencyManager crea un encargado de agencia sin contraseña inicial (solo admin).
func (h *UsuarioHandler) CreateAgencyManager(w http.ResponseWriter, r *http.Request) {
	var req models.CreateAgencyManagerRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		utils.ErrorResponse(w, "INVALID_JSON", "JSON invalido", nil, http.StatusBadRequest)
		return
	}
	...
	normalizedPhone, ok := validatePhone(req.Telefono)
	if !ok {
		utils.ErrorResponse(w, "VALIDATION_ERROR", "El telefono es invalido", nil, http.StatusBadRequest)
		return
	}
	req.Telefono = normalizedPhone
	...
	if err := database.GetDB().First(&agencia, req.AgenciaID).Error; err != nil {
		utils.ErrorResponse(w, "AGENCIA_NOT_FOUND", "Agencia no encontrada", nil, http.StatusNotFound)
		return
	}
	...
	expiresAt := time.Now().Add(time.Duration(utils.GetEnvInt("OTP_EXPIRY_MINUTES", 10)) * time.Minute)
	now := time.Now()
	...
	if err := database.GetDB().Transaction(func(tx *gorm.DB) error {
		...
		agencia.EncargadoPrincipalID = &usuario.ID
		...
		utils.ErrorResponse(w, "DB_ERROR", "Error al crear encargado", err.Error(), http.StatusInternalServerError)
		return
	}

	emailService := services.NewEmailService()
	if err := emailService.SendAgencyManagerWelcome(usuario.Email, code, agencia.NombreComercial); err != nil {
		log.Printf("Error enviando email de bienvenida: %v", err)
	}

	utils.SuccessResponse(w, map[string]interface{}{
		"user_id": usuario.ID,
	}, "Encargado de agencia creado exitosamente. Se envio un correo para activar la cuenta.", http.StatusCreated)
}
