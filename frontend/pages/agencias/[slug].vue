  ...
  <div class="page-shell">
    ...
    <div v-else-if="error" class="max-w-7xl mx-auto px-4 py-16 text-center space-y-4">
      ...
      <div class="flex justify-center gap-2">
        <Button label="Volver al inicio" icon="pi pi-home" severity="secondary" outlined @click="navigateTo('/')" />
    ...
    <div v-else-if="agencia">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="space-y-6">
            ...
            <Card v-if="hasRedes" class="surface-card">
              ...
              <template #content>
                <div class="space-y-3 text-sm">
                  <a v-if="agencia.sitio_web" :href="toExternalUrl(agencia.sitio_web)" target="_blank" rel="noopener" class="block text-blue-700 font-semibold hover:underline">
                    Sitio web
                  </a>
                  <a v-if="agencia.facebook" :href="toExternalUrl(agencia.facebook)" target="_blank" rel="noopener" class="block text-blue-700 font-semibold hover:underline">
                    Facebook
                  </a>
                  <a v-if="agencia.instagram" :href="toExternalUrl(agencia.instagram)" target="_blank" rel="noopener" class="block text-blue-700 font-semibold hover:underline">
                    Instagram
                  </a>
                </div>
              </template>
      ...
      <div ref="paquetesSection" class="scroll-mt-24 max-w-7xl mx-auto px-4 pb-10">
        <Card class="surface-card">
          <template #title>
            <div class="flex items-center gap-2">
...
import { computed, onMounted, ref, defineAsyncComponent, useHead } from '#imports'
...
const { registrarVisita } = useAgenciaVisitas()
const { getShareUrl } = useAgenciaSlug()
...
const loadAgencia = async () => {
  ...
  agencia.value = null
  try {
    const response: any = await getAgencia(slug.value)
    if (response.success) {
      ...
      // Registrar visita
      await registrarVisita(slug.value)
...
const setupMetaTags = () => {
  ...
  const title = `${agencia.value.nombre_comercial} - Andaria`
  const description = agencia.value.descripcion || `Conoce los servicios y paquetes de ${agencia.value.nombre_comercial}`
  const shareUrl = getShareUrl(slug.value)
  const imageUrl = heroFoto.value || ''
  ...
  useHead({
    ...
    meta: [
      ...
      { property: 'og:site_name', content: 'Andaria' },
