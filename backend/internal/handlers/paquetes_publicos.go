...
func (h *AgenciaHandler) GetPaquetesPublicos(w http.ResponseWriter, r *http.Request) {
	...
	agenciaIDStr := strings.TrimSpace(r.URL.Query().Get("agencia_id"))
	...
	if agenciaIDStr != "" {
		agenciaID, err := strconv.ParseUint(agenciaIDStr, 10, 32)
		if err != nil || agenciaID == 0 {
			utils.ErrorResponse(w, "VALIDATION_ERROR", "agencia_id invalido", nil, http.StatusBadRequest)
			return
		}
		query = query.Where("paquetes_turisticos.agencia_id = ?", uint(agenciaID))
	}
