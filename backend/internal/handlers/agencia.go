...
// generateUniqueSlug genera un slug único para una agencia
func generateUniqueSlug(nombreComercial string) (string, error) {
	baseSlug := utils.GenerateSlug(nombreComercial)
	slug := baseSlug
	counter := 1

	// Verificar si el slug existe
	for {
		var existingAgencia models.AgenciaTurismo
		err := database.GetDB().Where("slug = ?", slug).First(&existingAgencia).Error
		if err != nil {
			// Si no se encontró, el slug está disponible
			break
		}
...
// GetAgencia obtiene una agencia por ID o slug
...
func (h *AgenciaHandler) CreateAgenciaRapida(w http.ResponseWriter, r *http.Request) {
	...
	// Generar slug único
	slug, err := generateUniqueSlug(req.NombreComercial)
	if err != nil {
		utils.ErrorResponse(w, "SLUG_ERROR", "Error al generar slug", err.Error(), http.StatusInternalServerError)
		return
	}
