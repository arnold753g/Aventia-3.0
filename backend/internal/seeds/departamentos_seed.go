...
func shouldSeedDemoUsers() bool {
	seedEnv := strings.TrimSpace(os.Getenv("SEED_DEMO_USERS"))
	appEnv := strings.ToLower(strings.TrimSpace(os.Getenv("APP_ENV")))

	if appEnv == "production" {
		return isTruthy(seedEnv)
	}
...
func RunAllSeeds(db *gorm.DB) error {
	...
	if err := SeedAdminFromEnv(db); err != nil {
		return fmt.Errorf("error en seed de admin: %w", err)
	}
	...
	if shouldSeedDemoUsers() {
		...
		log.Println("Demo users seed skipped")
	}
